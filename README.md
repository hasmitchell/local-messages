...
The user has successfully sent an SMS from the composer, received a reply and used reactions. They report noticeably faster send confirmation with version 0.4. Attachment delivery and RCS sending still need device testing. Starting new conversations, typing indicators and production sync hardening remain subsequent milestones.
...
- The sidebar lists **Inbox** or **Archived** conversations with avatars, relative dates, two-line previews, an unread marker and any unsent draft. A conversation counts as unread while the phone reports it unread and it has not been opened here; the Dock icon shows the count (General settings can turn that off). Reading on the Mac never changes the phone's read status.
...
- The search field at the top of the sidebar searches conversation names, numbers and message text across the archive, with matching terms highlighted. The toolbar magnifying glass (**⌘F**) opens a find bar for the open conversation; **Esc** closes it. Results open the matching message in context, including older history.
- Click a downloaded photo or file for native Quick Look. Shared `.vcf` contacts open a local preview with selectable phone numbers, email and postal addresses. **Open in Contacts…** hands the file to macOS Contacts; previewing alone does not import anyone. The context menu can reveal the original in Finder.
- **⌘F** finds in the open conversation, **⌘⇧F** focuses the sidebar search (macOS 15 or later), **⌘I** toggles the conversation inspector, **⌘J** jumps to the latest message, **⌘,** opens Settings, **⌘O** opens another archive, and **⌘R** reloads it. The **Conversation** menu lists these.
...
- The toolbar's **ⓘ** button (or **⌘I**) opens an inspector beside the conversation with the contact's saved numbers and its **Photos**, **Links** and **Files** library. It stays open while you switch conversations. Shared items link back to their original messages; libraries initially scan 300 saved messages and offer more history when needed. Website previews are not fetched.
...
The composer sends text to an existing conversation only when you select Send or press **⌘Return**. Return adds a new line. Reading a conversation does not change its phone read status. Non-downloaded attachments show an explicit placeholder, and threads outside the saved date range have an empty-history explanation.
...
Choose an existing conversation and type in the composer. **Return** adds a line; **⌘Return** or the arrow button sends. Each conversation keeps its draft across switching and restarts, under a user-only `drafts/` directory inside the archive. The phone supplies the current outgoing participant and SIM; unavailable or read-only conversations fail before submission. SMS/RCS selection follows the phone's settings. Creating new conversations is not implemented yet.
...
Inbox and archive metadata are checked on startup and approximately every two minutes; recently active threads (past seven days) are refreshed even when their activity timestamp is unchanged. This is **not an exhaustive audit of old edits made while offline**. Missing remote rows, deleted conversations, spam/blocked transitions and orphaned media still need complete reconciliation. The bounded conversation-list limitation described below also applies to live sync. Initial catch-up can take minutes on a large phone history; local browsing stays available throughout.

Original photos and shared contact cards run in a separate bounded batch, checked every five minutes, up to 256 MiB of new downloads per batch in the app and 64 MiB per attachment. Text receiving continues while those downloads run. These are fetch limits, not a total disk-storage quota. Message bodies/reactions are preserved when an older attachment download finishes. The app uses `photos-and-contacts`; other attachment types remain metadata-only.
